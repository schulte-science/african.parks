...
app_server <- function(input, output, session) {
  ...
  rv <- list(
    ...
    meta = read.csv(app_sys("Metadata_20250519_JM.csv"), na.strings = c("", "NA")) |>
      dplyr::mutate(latitude = ifelse(latitude < -1000405, NA, latitude)),

    vert = read.csv(app_sys("12S_Summary_20250519_JM.csv"), na.strings = c("", "NA")),

    trnl = read.csv(app_sys("trnL_Summary_20250519_JM.csv"), na.strings = c("", "NA")) |>
      dplyr::filter(DNA_Diet_Host %in% c("Herbivore", "Omnivore")),
